...
func TestAuthFileCodexStatsAddsCardBatchSearchHelper(t *testing.T) {
	...
	assertContains(t, script, "function shouldResolveCardBatchSearch(parsed)")
	assertContains(t, script, "return terms.length === 1 && isLikelyCardCodeTerm(terms[0]);")
	assertContains(t, script, "if (!shouldResolveCardBatchSearch(parsed))")
	assertContains(t, script, "function clearStaleCardBatchSearch(sourceInput, helper)")
