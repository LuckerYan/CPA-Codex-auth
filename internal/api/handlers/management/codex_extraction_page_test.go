...
func TestCodexExtractionPageSupportsBatchCodes(t *testing.T) {
	...
	assertCodexExtractionPageNotContains(t, "id=\"extractConcurrency\"")
	assertCodexExtractionPageNotContains(t, "并发验活 / 提取")
	assertCodexExtractionPageNotContains(t, "concurrency-row")
	assertCodexExtractionPageNotContains(t, "concurrency-input")
	assertCodexExtractionPageContains(t, "function getExtractConcurrency()")
	assertCodexExtractionPageContains(t, "return 10;")
	assertCodexExtractionPageNotContains(t, "function syncExtractConcurrency()")
	assertCodexExtractionPageContains(t, "JSON.stringify({ items: codes, format: format, concurrency: concurrency })")
	assertCodexExtractionPageNotContains(t, "concurrencyInput.addEventListener('change', syncExtractConcurrency)")
	...
	assertCodexExtractionPageContains(t, ".result-modal.is-open")
	assertCodexExtractionPageContains(t, ".result-modal.is-closing")
	assertCodexExtractionPageContains(t, "prefers-reduced-motion: reduce")
	assertCodexExtractionPageContains(t, "requestAnimationFrame(function ()")
	assertCodexExtractionPageContains(t, "finishResultModalClose")
	assertCodexExtractionPageContains(t, "resultHideTimer")
	assertCodexExtractionPageNotContains(t, "id=\"status\"")
	assertCodexExtractionPageNotContains(t, "function setStatus")
	assertCodexExtractionPageContains(t, "event.ctrlKey || event.metaKey")
	assertCodexExtractionPageContains(t, "请先输入卡密或提取链接")
	assertCodexExtractionPageContains(t, "提取部分成功")
}
